...
[
  ...
  disease-file-store
  schedule-file-store
  demographic-file-store-humans
  demographic-file-store-humans-2
  demographic-file-store-ambulance
  output-file-path
  house1
  house2
  house3
...
[
  ...
  ;24
  curren-action ;; the current list of actions
  ;25
  house-patch
  ;26
  age
...
to setup
  ...
  file-open "C:\\Users\\user\\Desktop\\Repos\\epiDem_Tc_replication\\enriched\\inputFiles\\travelDis.json"
  ;;set disease-store

    set disease-file-store file-read-characters 99999999
  file-close

  file-open "C:\\Users\\user\\Desktop\\Repos\\epiDem_Tc_replication\\enriched\\inputFiles\\Schedule.json"

  set schedule-file-store file-read-characters 99999999

  file-close

  file-open "C:\\Users\\user\\Desktop\\Repos\\epiDem_Tc_replication\\enriched\\inputFiles\\travelPopulationDemographics.json"

  set demographic-file-store-humans file-read-characters 99999999

  file-close

  file-open "C:\\Users\\user\\Desktop\\Repos\\epiDem_Tc_replication\\enriched\\inputFiles\\travelPopulationDemographics2.json"

  set demographic-file-store-humans-2 file-read-characters 99999999

  file-close

  set output-file-path "C:\\Users\\user\\Desktop\\Repos\\epiDem_Tc_replication\\enriched\\inputFiles\\outputFiles\\"

  file-open "C:\\Users\\user\\Desktop\\Repos\\EpiLogo paper replicatio\\extended\\inputFiles\\travelAmbulanceDemographics.json"

  set demographic-file-store-ambulance file-read-characters 99999999
  ...
  setup-people
  setup-ambulance
  set initial-people count turtles
  setup-disease
  ask turtles with [ambulance? = false][
    setup-population-visuals
    setup-inoculation
    if links? [ setup-network]
      get-current-actions
  ]

  ;correcting ambulance psotions
  ask turtles with[ambulance? = true][
    setup-population-visuals
    setup-inoculation
   ifelse continent = 1[
...
to setup-globals
  ...
  set border patches with [(pxcor =  0 and ((pycor > 1 and pycor < 12) or (pycor < -1 and pycor > -12)))]
  ...
  ;----------------------------------------------------------------------------
  set house1 patches with[(pxcor < -4 and pxcor > -8) and (pycor < -4 and pycor > -8)]
  ask house1 [set pcolor green]

  set house2 patches with[(pxcor > 4 and pxcor < 8) and (pycor > 4 and pycor < 8)]
  ask house2 [set pcolor green]

 set school patches with[(pxcor > 4 and pxcor < 8) and (pycor < -4 and pycor > -8)]
  ask school [set pcolor blue]

  set work patches with[(pxcor < -4 and pxcor > -8) and (pycor > 4 and pycor < 8)]
  ask work [set pcolor grey]

  ask patch (- max-pxcor / 2 ) 0 [ set pcolor white ]
  ask patch (max-pxcor / 2 ) 0 [ set pcolor white ]

  set hospital patches with [pcolor = white]
...
to setup-people
  ;Sprout the population demographics with no attributes
  let a epiLogo:input-new-demographic demographic-file-store-humans output-file-path -6 -6 5 false
  ;----------------------------------------------------------------
  let b epiLogo:input-new-demographic demographic-file-store-humans-2 output-file-path 6 6 5 false
  ;-----------------------------------------------------------------
  ask turtles with [ambulance? = false]   [
   ;------------------------------------------------------
    if color = red[
    ...
    assign-tendency
      ...
      set house-patch patch-here
    set school-patch one-of school
    set work-patch one-of work
    set current-task "home"
...
to setup-ambulance
  let a epiLogo:input-new-demographic demographic-file-store-ambulance output-file-path 1 0 577 false
end


to setup-disease

  let d epiLogo:initialize-disease disease-file-store  turtles false
end

to setup-population-visuals
   ifelse xcor <= 0
      [ set continent 1
      set shape "square"]
      [ set continent 2
      set shape "circle" ]
  ifelse (ambulance? = true)[
    set shape "person"
    set color yellow
...
to setup-inoculation
  ifelse (color = 9.9) and (random-float 100 < inoculation-chance)
        [ set inoculated? true]
        [set inoculated? false]

  ask turtles with [inoculated? = true] [
...
to go
if all? turtles [ color = blue or color = green or color = white or color = yellow ]
    [ stop ]
 ...
 ask turtles
  ...
  ask turtles with [ambulance? = false][
    ...
    ifelse member? "complete" curren-action [

      set task-completed false

      set current-task item 1 curren-action
...
if current-task = "work"[
    ...
    if member? "hospital" curren-action[
    ...
    if member? "move" curren-action[

     move
    ]

    if member? "infect" curren-action[

     infect
    ]

    if member? "recover" curren-action[
...
to move  ;; turtle procedure
  ...
  set path []
  let obstacles patches with[passable = false]


ifelse ambulance? = false[

  if current-task = "home"
  [

    if patch-here != house-patch[

    set path epiLogo:find-path patch-here house-patch obstacles false
    face item 0 path
     fd intra-mobility
    ]
  ]
...
if current-task = "school"
  [
    ...
    if patch-here != school-patch[
      set path epiLogo:find-path patch-here school-patch obstacles false
    face item 0 path
     fd intra-mobility
    ]
  ]
 ...
 if patch-here != work-patch[
      set path epiLogo:find-path patch-here work-patch obstacles false
    face item 0 path
     fd intra-mobility
    ]
  ]
...
[
;let end-patch one-of patches
;set path epiLogo:find-path patch-here end-patch obstacles false

    if count turtles with [color = red] != 0[

    face one-of turtles with [color = red]
...
to infect  ;; turtle procedure
    ...
    with [ color = 9.9 and not inoculated? ]
    ...
    [
       ...
       [
           if random-float 1 < 0.1[
               ...
               let a epiLogo:disease-spread disease-file-store "far" self
      ]
    ]
    ]
...
to maybe-recover
  ...
  ifelse current-task != "hospital"[

            let a epiLogo:evaluate-recovery  disease-file-store "nonHospital" self
  ]
  [
    let a epiLogo:evaluate-recovery  disease-file-store "hospital" self
  ]
...
to get-current-actions
  ...
  ;;get the action of agents

    let l [ ]

    set l lput "beahvioursPerson" l

  ifelse task-completed = true [
      set l lput "completed" l
    ]
    [
      set l lput "nonCompleted" l
    ]


  ifelse current-task = "home" [
      set l lput "home" l
    ]
    [
      ifelse current-task = "school"[
  ...
  [
      ...
      ifelse current-task = "work" [
      set l lput "poi" l
      ]
      [
        if hospitalized? = true[

        set l lput "hospital" l
        ]
      ]
  ...
  ifelse age = "A" [
   ...
   ifelse color = red[
    set l lput "infected" l

  ]
  [
    set l lput "nonInfected" l
  ]



    set curren-action epiLogo:get-current-task schedule-file-store l
end
@#$#@#$#@
GRAPHICS-WINDOW
646
...
"" ""
PENS
"i" 1.0 0 -2674135 true "" "plot count turtles with [ color = 15 ]"
"s" 1.0 0 -10899396 true "" "plot count turtles with [  color = 9.9 and not inoculated? and not ambulance? ]"
"R" 1.0 0 -7500403 true "" "plot count turtles with [ color = 55 or color = 105 and not ambulance?  ]"
...
"" ""
PENS
"% infected" 1.0 0 -2674135 true "" "plot (((count turtles with [ color = 55 ] + count turtles with [ color = 15 ]) / initial-people) * 100)"
"% recovered" 1.0 0 -10899396 true "" "plot ((count turtles with [ color = 55 ] / initial-people) * 100)"
...
@#$#@#$#@
<experiments>
  <experiment name="condor" repetitions="2000" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>ticks</metric>
    <metric>count turtles with [color = 9.9 and not inoculated? and not ambulance?]</metric>
    <metric>count turtles with [color = 15]</metric>
    <metric>count turtles with [color = 55 or color = 105 and not ambulance?]</metric>
    <metric>timer</metric>
    <enumeratedValueSet variable="average-hospital-going-tendency">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="travel-tendency">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-recovery-time">
      <value value="110"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="inoculation-chance">
      <value value="10"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="links?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="travel?">
      <value value="true"/>
    </enumeratedValueSet>
