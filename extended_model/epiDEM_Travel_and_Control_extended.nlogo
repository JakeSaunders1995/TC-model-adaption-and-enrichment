...
to setup
  ...
  file-open "C:\\Users\\user\\Desktop\\Repos\\EpiLogo paper replicatio\\extended\\inputFiles\\travelDis.json"
  ;;set disease-store

    set disease-file-store file-read-characters 99999999
  file-close

  file-open "C:\\Users\\user\\Desktop\\Repos\\EpiLogo paper replicatio\\extended\\inputFiles\\travelSchedule.json"

  set schedule-file-store file-read-characters 99999999

  file-close

  file-open "C:\\Users\\user\\Desktop\\Repos\\EpiLogo paper replicatio\\extended\\inputFiles\\travelPopulationDemographics.json"
  ...
  set output-file-path "C:\\Users\\user\\Desktop\\Repos\\EpiLogo paper replicatio\\extended\\outputFiles\\"
...
to setup-people
  let a epiLogo:input-new-demographic demographic-file-store-humans output-file-path 1 0 577 false
 ...
 ask turtles
  ...
  ask turtles with [ambulance? = false] [
   ...
   if member? "move" curren-action [
    ...
    if member? "isolate" curren-action and (random 100 < isolation-tendency)[
      isolate
    ]

    if member? "hospitalize" curren-action and (random 100 < hospital-going-tendency)[
      hospitalize
    ]
  ]
 ...
 ask turtles
  [
    ...
    [
      ...
      [
        if (ambulance? = false) and (color = red)
        [ hospitalize ]
      ]
    ]
  ]
  ...
  ask turtles[
    if member? "unisolate" curren-action[
      unisolate
    ]
  ]
tick
...
to move  ;; turtle procedure
  ...
  let path []
  let obstacles patches with [pcolor = pink]
  ...
  [
    ...
    let end-patch one-of neighbors
...
to infect  ;; turtle procedure
    ...
    [
       ...
       [
           ...
           [
               let a epiLogo:disease-spread disease-file-store "linked" self
        if (color = 15)[set nb-infected (nb-infected + 1)]


           ]
           [
            let a epiLogo:disease-spread disease-file-store "non-linked" self
            if (color = 15)[set nb-infected (nb-infected + 1)]
...
to maybe-recover
  set infection-length infection-length + 1
      ...
      if (color = 55)[set nb-recovered (nb-recovered + 1)]
...
to get-current-actions
    ...
    set l lput "SchedulePerson" l

    ifelse isolated? [
     set l lput "Isolated?" l

    ]
    [
      set l lput "NonIsolated?" l
    ]

    ifelse hospitalized? [
      set l lput "Hospitalized?" l
    ]
    [
      set l lput "NonHospitalized?" l
    ]
